...
impl InferenceCore {
    ...
    pub fn decode(
        ...
        let synthesis_system = model_config
            .synthesis_system
            .unwrap_or("fs2+fregan".to_string());
        let period_jets_mode = synthesis_system == "period-jets";
        ...
        if synthesis_system == "fs2+fregan" {
        ...
        } else if period_jets_mode {
            embedder_input_tensors = vec![&mut phoneme_vector_array, &mut speaker_id_array];
        } else {
        ...
        // decoder_input_tensorsが初期化されない可能性を排除するため、elseを用いる
        // } else if period_jets_mode {
        } else {
